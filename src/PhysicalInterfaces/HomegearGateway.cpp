...
int32_t HomegearGateway::setBaseAddress(uint32_t value)
{
    try
    {
        ...
        //Todo: Implement
        _out.printError("Error: Method is not implemented yet for Homegear Gateway.");
    }
    catch(const std::exception& ex)
    {
        _out.printEx(__FILE__, __LINE__, __PRETTY_FUNCTION__, ex.what());
...
void HomegearGateway::listen()
{
    try
    {
        ...
        {
            _tcpSocket->open();
            if(_tcpSocket->connected()) _out.printInfo("Info: Successfully connected.");
        }
        catch(BaseLib::Exception& ex)
        {
            _out.printEx(__FILE__, __LINE__, __PRETTY_FUNCTION__, ex.what());
...
PVariable HomegearGateway::invoke(std::string methodName, PArray& parameters)
{
    try
    {
        ...
        _rpcEncoder->encodeRequest("sendPacket", parameters, encodedPacket);
